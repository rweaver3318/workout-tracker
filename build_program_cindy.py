...
Mapping:
  PF levels 0–5          → levelSystem.levels (set manually in Settings)
...
PROGRAM = {
    ...
    "painRules": {
        ...
        "hold": "Pain >3 during, OR not settled in 24 h, OR first-step worse, OR knee more swollen → HOLD PF level / reduce knee load; DROP a PF level after 2 fails",
        "advance": "2–3 consecutive PASS sessions to advance a PF level",
        "kneeBand": "Knee band moves up every 2 weeks only if most sessions in the band were PASS; otherwise repeat the band",
        "pickleball": "Pickleball = heel load. If first-step pain ≥7/10 (2+ above baseline) for 2 days, HOLD PF level and trim court time first",
    ...
    "sessionTypes": {
        "H": "PF level + Knee band + Hips/Glutes",
        "S": "PF level + Knee band (◆ only) + Shoulders",
        "M": "PF level + Knee band + Mixed (2 hip + 2 shoulder)",
        "R": "Week 8 reassessment (light PF + tests)",
    },
    "optionalTypes": [],
    "notes": [
        "PF (left heel) = symptom-based levels 0–5. Start Level 0; stay double-leg through Level 3; don't skip levels.",
        ...
        "Pickleball = heel load: log first-step heel pain the morning after play. ≥7/10 two days running → hold PF level, trim court time first.",
...
PF_LEVELS = [
    {"level": 0, "name": "Gentle activation", "estimate": "Weeks 1–2",
     ...
     "exercises": [
         ex("pf0-seated-raise", "Seated heel raises (both feet)", "2", reps="15", load="Bodyweight · 2-1-2", notes="Knees bent ~90°."),
    ...
    {"level": 1, "name": "Loaded seated + supported standing", "estimate": "Weeks 2–3",
     ...
     "exercises": [
         ex("pf1-seated-calf", "Seated calf raise (soleus)", "3", reps="12–15", load="10–20 lb DB on knees · 2-1-2", notes="Both feet."),
         ex("pf1-standing-dl", "Standing double-leg heel raise, flat floor", "2", reps="10", load="Bodyweight · 2-1-2", notes="Hands on counter/rack."),
    ...
    {"level": 2, "name": "Standing double-leg strength", "estimate": "Weeks 3–4",
     ...
     "exit": "2–3 PASS; 3×15 standing DL raises through full range with control.",
     "exercises": [
         ex("pf2-dl-straight", "Standing DL heel raise (straight knee)", "3", reps="12–15", load="Bodyweight · 3-0-3", notes="Light hand support."),
         ex("pf2-dl-bent", "Standing DL heel raise (bent knee ~20–30°)", "3", reps="10–12", load="Bodyweight · 3-0-3", notes="Soleus bias; knees over toes."),
         ex("pf2-seated-calf", "Seated calf raise", "3", reps="12", load="20–30 lb · 2-1-2", notes=""),
    ...
    {"level": 3, "name": "Double-leg Rathleff-style raise (towel under toes)", "estimate": "Weeks 4–5",
     ...
     "exercises": [
         ex("pf3-rathleff-dl", "DL heel raise on step edge, rolled towel under toes", "3", reps="12",
            load="Bodyweight → light backpack", notes="3 s up, 2 s hold, 3 s down. Heels drop only to comfortable depth.",
            cues="3-2-3 tempo."),
         ex("pf3-bent-knee", "Bent-knee DL heel raise", "3", reps="12", load="Hold 10–15 lb DB · 3-0-3", notes=""),
         ex("pf3-seated-calf", "Seated calf raise", "3", reps="12", load="30 lb · 2-1-2", notes=""),
     ]},
    {"level": 4, "name": "Single-leg transition (assisted)", "estimate": "Weeks 5–6",
     "entry": "Level 3 exit met.",
     "exit": "2–3 PASS; 3×12 unassisted SL towel raises on the left, pain ≤2–3/10.",
     "exercises": [
         ex("pf4-rathleff-sl-l", "Rathleff heel raise (LEFT), assisted", "3", reps="12RM", load="Bodyweight · 3-2-3",
         ...
         ex("pf4-sl-r", "Right side for balance", "3", reps="12", load="3-2-3", notes="Match both sides."),
         ex("pf4-bent-knee", "Bent-knee heel raise (DL)", "3", reps="12", load="15–20 lb · 3-0-3", notes=""),
     ]},
    {"level": 5, "name": "Full Rathleff high-load protocol", "estimate": "Weeks 6–8+",
     ...
     "exercises": [
         ex("pf5-rathleff", "Rathleff SL heel raise (towel, step edge)", "3 / 4 / 5", reps="12RM / 10RM / 8RM",
            load="Backpack or DB in same-side hand",
            notes="Stage 5a 3×12RM → 5b 4×10RM → 5c 5×8RM. 3 s up / 2 s hold / 3 s down. Move stages only after 2–3 PASS at each. Log your stage in the note.",
            flag=T, flag_note="Heel irritability and balance – stays double-leg if first-step pain is high."),
...
BASELINE = [
    ex("cb-chair-stand", "Baseline: 30-s chair stand (standard chair, arms crossed)", "1", reps="max in 30 s", load="BW", notes="Record reps."),
    ex("cb-dl-raise", "Baseline: DL heel raises (3-0-3) to fatigue", "1", reps="max", load="BW", notes="Record reps."),
...
REASSESSMENT = [
    ...
    ex("cr-heel-raise", "Heel raise test: max DL (3-0-3) and max SL each side", "1", reps="max", load="BW", notes=""),
    ex("cr-chair-stand", "30-s chair stand", "1", reps="max in 30 s", load="BW", notes="Standard chair, arms crossed."),
    ex("cr-walk", "40-m fast-paced walk (10 m × 4)", "1", reps="time", load="—", notes="Record seconds."),
    ex("cr-stepup", "Step-up: highest step 8×/side, good alignment", "1", reps="8/side", load="BW", notes=""),
    ex("cr-sl-stand", "Single-leg stand each side", "1", hold="max 30 s", load="—", notes=""),
    ex("cr-shoulders", "Shoulders: band ER 15/side; scaption 10 at current load", "1", reps="as listed", load="Band / DB", notes="Pain-free."),
...
KNEE = {
    "A": [
        ex("ka-sts-high", "Sit-to-stand, HIGH seat (chair + cushion)", "3", reps="8", load="3 s down / 1 s up · BW",
           notes="Knees over 2nd toe; hands free if possible.", diamond=True),
        ex("ka-knee-ext", "Seated knee extension, band", "2", reps="10/side", load="2-1-3 · light band",
           notes="Band anchored to chair leg.", diamond=True),
        ex("ka-ham-curl", "Standing hamstring curl, band", "2", reps="10/side", load="2-1-2 · light band", notes="Hold counter."),
        ex("ka-wall-sit", "Wall sit, shallow (~30–45°)", "3", hold="20 s", load="Isometric", notes="Pain ≤2–3/10."),
    ],
    "B": [
        ex("kb-sts", "Sit-to-stand, standard chair", "3", reps="10", load="3-0-1 · BW → 10 lb goblet", notes="", diamond=True),
        ex("kb-stepup-low", "Step-up, low step (~4–6\")", "2", reps="8/side", load="Controlled", notes="Rail/wall support; knee over foot."),
        ex("kb-knee-ext", "Seated knee extension, band", "3", reps="12/side", load="2-1-3 · medium band", notes="", diamond=True),
        ex("kb-ham-curl", "Hamstring curl, band (standing or seated)", "3", reps="10/side", load="2-1-2", notes=""),
    ...
    "C": [
        ex("kc-box-squat", "Box squat to bench", "3", reps="8", load="3-1-1 · 10–15 lb goblet",
           notes="Touch bench lightly, don't collapse.", diamond=True, flag=T, flag_note=SQUAT_NOTE),
        ex("kc-stepup", "Step-up, low → higher step (~6–8\")", "3", reps="8/side", load="BW → 10 lb DBs",
           notes="Controlled.", flag=T, flag_note=STEP_NOTE),
        ex("kc-knee-ext", "Seated knee extension, band or DB between feet", "3", reps="12", load="2-1-3 · heavier band", notes="", diamond=True),
        ex("kc-ham-curl", "Hamstring curl, band", "3", reps="12/side", load="2-1-2", notes=""),
        ex("kc-spanish", "Spanish squat / wall sit", "4", hold="30–45 s", load="Isometric", notes=""),
    ],
    "D": [
        ex("kd-goblet", "Goblet squat to comfortable depth (≈ bench height)", "3", reps="8", load="3-1-1 · 15–20 lb",
           notes="Knees over toes; stop before pain. Not deep.", diamond=True, flag=T, flag_note=SQUAT_NOTE),
        ex("kd-stepup", "Step-up, 8\"", "3", reps="8/side", load="10–15 lb DBs", notes="", flag=T, flag_note=STEP_NOTE),
        ...
        ex("kd-knee-ext", "Knee extension (band/DB)", "3", reps="12", load="2-1-3", notes="", diamond=True),
        ex("kd-ham-curl", "Hamstring curl, band", "3", reps="12", load="2-1-2", notes=""),
...
SHOULDERS = {
    "1-2": [
        ...
        ex("cs12-scaption", "Scaption to shoulder height (thumbs up, slight V)", "2", reps="10", load="None / water bottles", notes=""),
    ],
    "3-4": [
        ex("cs34-erir", "Band ER / IR", "3", reps="12/side", load="Light–medium band", notes=""),
        ...
        ex("cs34-scaption", "Scaption", "3", reps="10", load="Water bottles or half-gallon jug", notes=""),
        ex("cs34-wallpush", "Wall push-up", "2", reps="8–10", load="Bodyweight", notes=""),
    ],
    "5-6": [
        ex("cs56-erir", "Band ER / IR", "3", reps="12–15/side", load="Medium band", notes=""),
        ex("cs56-dbrow", "One-arm DB row (hand on bench)", "3", reps="10/side", load="10–15 lb", notes=""),
        ex("cs56-facepull", "Face pull", "3", reps="12", load="Medium band", notes=""),
        ex("cs56-scaption", "Scaption", "3", reps="10", load="10 lb DBs or gallon jugs", notes="Stop at shoulder height."),
        ex("cs56-incline-push", "Incline push-up (bench or rack bar)", "3", reps="8", load="Bodyweight", notes=""),
    ],
    "7-8": [
        ex("cs78-er45", "Band ER at 45° abduction", "3", reps="12/side", load="Medium band", notes="Pain-free."),
        ex("cs78-dbrow", "DB row", "3", reps="10/side", load="15–20 lb", notes=""),
        ex("cs78-ohp", "Seated DB overhead press (light)", "2", reps="8", load="10 lb", notes="Pain-free range only."),
...
HIPS = {
    ...
    "3-4": [
        ex("ch34-bridge", "Glute bridge", "3", reps="12", load="BW → band above knees", notes=""),
        ...
        ex("ch34-rdl", "Hip hinge → DB RDL", "3", reps="8", load="Dowel → 10 lb DBs", notes="Soft knees, flat back."),
    ],
    "5-6": [
        ex("ch56-rdl", "DB Romanian deadlift", "3", reps="8–10", load="15–20 lb DBs", notes=""),
    ...
    "7-8": [
        ex("ch78-rdl", "DB RDL", "3", reps="8", load="20–30 lb DBs", notes="Or light barbell if form is excellent."),
        ex("ch78-thrust", "Hip thrust (upper back on bench)", "3", reps="10", load="BW → 20 lb DB on hips", notes="Pad under DB."),
...
SCHEDULE_ROWS = [
    ("2026-09-27", "Sun", 1, "A", "H", "H — Session 1 (PF Level 0 start; record baseline)"),
    ...
    ("2026-11-20", "Fri", 8, "D", "R", "R — Week 8 reassessment (Session 24; light PF + tests)"),
]
...
ANSWER_DEPENDENT = [
    ...
    {"exercise": "Rathleff SL heel raise (Level 4–5)", "why": "Heel irritability and balance; stays DL if first-step pain is high"},
...
def build() -> dict:
    ...
    pf = {"title": "Foot — PF Level {level}: {levelName}", "source": "level"}
    ...
    data["sessionTemplates"] = {
        ...
        "R": [daily,
              {"title": "Light PF — Level {level} (first 2)", "source": "level", "first": 2},
    ...
    data["conditioningNotes"] = [
        ...
        "Treadmill: walking only, no incline early; small incline after PF Level 3 (†).",
